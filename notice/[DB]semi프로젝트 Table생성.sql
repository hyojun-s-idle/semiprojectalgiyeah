...
SELECT B.BOARD_NO,B.BOARD_TITLE,B.BOARD_CONTENT, BT.BOARD_NAME CATEGORY, BT2.BOARD_NAME PR_BOARD,
    (SELECT COUNT(*) FROM REPORT WHERE BOARD_NO = REPORT_NO AND REPORT_TYPE = 1) RCOUNT,
    MEMBER_NICK,MEMBER_TYPE_NM,B.CREATE_DT
FROM BOARD B
...
SELECT A.* FROM(
SELECT BOARD_NO,
    (SELECT COUNT(*) FROM REPORT WHERE BOARD_NO = REPORT_NO AND REPORT_TYPE = 2) RCOUNT
FROM BOARD
JOIN BOARD_TYPE USING(BOARD_TYPE_CD)
WHERE BOARD_ST ='N'
ORDER BY RCOUNT DESC)A
...
-- 댓글 추가
select * from REPLY;
INSERT INTO REPLY VALUES(SEQ_RNO.NEXTVAL,'댓글 테스트',DEFAULT,DEFAULT,DEFAULT,2200,4,DEFAULT);
